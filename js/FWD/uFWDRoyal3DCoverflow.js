...
(function(window)
{
	var FWDR3DCovData = function(props)
	{
		...
		this.parseProperties = function()
		{
			...
			self.rootElement = FWDUtils.getChildById(self.propsObj.coverflowDataListDivId);
			...
			{
				self.dispatchEvent(FWDR3DCovData.LOAD_ERROR, {text:"Coverflow graphics skin path is not defined in FWD3DCoverflow constructor function!"});
				return;
			}
	...
	{
		FWDR3DCovData.prototype = new FWDEventDispatcher();
	};
...
(function(window)
{
	...
	{
		FWDR3DCovScrollbar.prototype = new FWDDisplayObject("div");
	};
...
(function(window)
{
	...
	{
		FWDR3DCovSlideshowButton.prototype = new FWDDisplayObject("div");
	};
...
(function(window)
{
	var FWDR3DCovThumb = function(id, data, parent)
	{
		...
		this.setScale = function(scale, alpha)
		{
			...
			if (!FWDUtils.hasTransform2d)
			{
				...
				TweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:-Math.floor(scaleH/2), w:scaleW, h:scaleH, alpha:alpha, ease:Quart.easeOut});
			...
			{
				TweenMax.to(self, .8, {x:Math.floor(self.newX), scale:self.thumbScale, alpha:alpha, ease:Quart.easeOut, onComplete:self.setThumbVisibility});
			}
		};
		...
		this.onMouseOverHandler = function()
		{
			if (!self.isOver2 && (self.id != parent.curId))
			{
				...
				TweenMax.to(self, .6, {z:self.curZ + 100, ease:Expo.easeOut});
			}
		};
		...
		this.onMouseOutHandler = function()
		{
			if (self.isOver2 && (self.id != parent.curId))
			{
				...
				TweenMax.to(self, .6, {z:self.curZ, ease:Expo.easeOut});
			}
		};
	...
	{
		FWDR3DCovThumb.prototype = new FWDDisplayObject3D("div", "absolute", "visible");
	};
...
(function(window)
{
	var FWDR3DCovThumbsManager = function(data, parent)
	{
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
				...
				if (i != self.curId)
				{
					if ((self.nrThumbsToDisplay > 0) && (Math.abs(i - self.curId) <= self.nrThumbsToDisplay))
					{
						...
						{
							TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleY:newAngleY, delay:delay/1000, ease:Quart.easeOut});
							thumb.setZIndex(self.nrThumbsToDisplay + 1 - Math.abs(i - self.curId));
						}
						else
						{
							TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleY:newAngleY, angleX:.1, delay:delay/1000, ease:Quart.easeOut});
						}
					}
					else
					{
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
				...
				if (Math.abs(thumb.curX) != Math.floor(self.thumbXSpace3D * (self.nrThumbsToDisplay + 1) + self.thumbXOffset3D))
				{
					...
					if ((self.nrThumbsToDisplay > 0) && (Math.abs(i - self.curId) <= self.nrThumbsToDisplay))
					{
						if (FWDUtils.isIEAndMoreThen9)
						{
							TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleY:newAngleY, alpha:1, ease:Quart.easeOut});
						}
						else
						{
							TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleY:newAngleY, angleX:.1, alpha:1, ease:Quart.easeOut});
						}
					...
					else
					{
						if (self.nrThumbsToDisplay > 0)
						{
							if (FWDUtils.isIEAndMoreThen9)
							{
								TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleY:newAngleY, alpha:0, ease:Quart.easeOut});
							}
							else
							{
								TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleY:newAngleY, angleX:.1, alpha:0, ease:Quart.easeOut});
							}
		...
		this.showControls = function()
		{
			if (self.data.controlsPos)
			{
				TweenMax.to(self.controlsDO, .8, {y:0, ease : Expo.easeOut});
			}
			else
			{
	...
	{
		FWDR3DCovThumbsManager.prototype = new FWDDisplayObject3D("div", "relative", "visible");
	};
...
(function(window)
{
	var FWDRoyal3DCoverflow = function(props)
	{
		...
		this.init = function()
		{
			...
			if (!FWDUtils.getChildById(self.propsObj.coverflowHolderDivId))
			{
			...
			self.stageContainer = FWDUtils.getChildById(self.propsObj.coverflowHolderDivId);
		...
		this.resizeHandler = function()
		{
			...
			else if (self.displayType == FWDRoyal3DCoverflow.AFTER_PARENT)
			{
		...
		this.setupPreloader = function()
		{
			...
			self.preloaderDO = new FWDPreloader(self.data.mainPreloaderImg, 85, 36, 20, 50);
		...
		this.onThumbsManagerIntroFinish = function()
		{
			...
			self.dispatchEvent(FWDRoyal3DCoverflow.CATEGORY_CHANGED);
		};
	...
	FWDRoyal3DCoverflow.AFTER_PARENT = "afterparent";
	...
	FWDRoyal3DCoverflow.CATEGORY_CHANGED = "categoryChanged";
