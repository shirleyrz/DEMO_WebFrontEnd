...
(function ($, undefined) {
    $.fn.drawDoughnutChart = function (data, options) {
        ...
        function pathMouseEnter(e) {
            ...
            if (data[order].display_text) {
                $tip.text((data[order].title != "" ? data[order].title + ": " : "") + data[order].value * 100 +"%")
                    .fadeIn(200);
        ...
        function drawPieSegments(animationDecimal) {
            ...
            //draw each path
            for (var i = 0, len = data.length; i < len; i++) {
                /* here we only show the original value, not the percentage we calculated*/
                var percentage = data[i].value;
                /*(data[i].value / segmentTotal)*/
                var segmentAngle = rotateAnimation * ( percentage *(PI * 2));
